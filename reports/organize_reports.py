#!/usr/bin/env python3
"""Self-contained organizer for this project's ``reports/`` folder.
...
Also drains the sibling ``report/`` folder (singular) — created in error — into
the same dated layout under ``reports/``, then prunes both trees of empty
directories deepest-first. Legacy ``YYYYMMDD`` staging folders (e.g.
``reports/20260418/``) are NOT recognized as already-organized, so their
contents are re-homed into ``reports/2026.04/2026.04.18/`` and the empty stub is
removed.

Internalized copy: this file carries the full move/prune logic inline so it has
no dependency on any other repository's shared module.
...
def _resolve_skip_paths(
    ...
    """Paths that must stay put (this organizer script, explicit overrides).

    Always skip the conventional ``reports/organize_reports.py`` when
    ``project_root`` resolves it — otherwise a run would relocate this committed
    organizer script alongside generated logs on Windows/Linux.
    """
...
def organize_reports(
    ...
    *,
    ...
    Also drains the sibling ``report/`` (singular) staging folder into the same
    layout. Returns ``(moved_count, skipped_count)``.
    """
    reports_root_r = reports_root.resolve()
    # The sibling `report/` folder was created in error; drain it into `reports/`.
    ...
    for folder in dirs_found:
        try:
            if any(folder.resolve() == root for root in roots):
                continue
            # Keep folders that still contain this organizer script.
            if any(
...
def organize_and_prune_reports(
    ...
    *,
    ...
    # Prune the drained sibling `report/` tree alongside `reports/`.
    legacy_report_root = reports_root.resolve().parent / "report"
    removed = remove_empty_directories(
        reports_root,
        extra_roots=(legacy_report_root,),
        print_removed=print_removed,
    )

    # remove_empty_directories protects every root it is given (so `reports/`
    # itself is never deleted), which leaves the now-drained `report/` shell
    # behind. That folder was created in error — once empty, delete it outright
    # rather than stranding an empty mistake at the repo root.
    if legacy_report_root.is_dir():
        try:
            if not any(legacy_report_root.iterdir()):
                legacy_report_root.rmdir()
                removed += 1
                if print_removed:
                    print(f"Removed empty: {legacy_report_root}")
        except OSError:
            pass
...
def main() -> int:
    ...
    moved, skipped, removed = organize_and_prune_reports(
        ...
        # Keep terminal output readable; detailed moved/skipped entries go to daily log.
        print_moves=False,
        print_removed=False,
    )
    print(
