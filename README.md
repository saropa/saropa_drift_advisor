...
| **Optional writes**          | **Import** (CSV/JSON/SQL) with column mapping; **data editing** with undo, SQL preview, **FK-aware batch apply** (`writeQuery`, **`POST /api/edits/apply`**); **Bulk Edit** grid webview (extension); **browser** inline Save/Cancel and single-row delete when writes are enabled and the table has one PK column; **seeding** and **clear** helpers (extension).                                                                                                                   |
...
- **Inline edits when writes are enabled** — if `/api/health` reports `writeEnabled` and the table has a **single** primary-key column, you can edit cells with explicit **Save/Cancel** (no blur commit), delete one row with confirmation, and get a prompt before leaving the page with unsaved edits; the row highlights when your draft differs from the loaded value, failed saves offer **Retry save** / **Reload table**, and a failed delete can reload the grid from a confirm dialog
...
| **[doc/EXTENSION_API.md](doc/EXTENSION_API.md)**                                        | VS Code extension API for other extensions, Suite daily-summary contract, and Log Capture sidecar file contract.        |
| **[doc/DESIGN_LANGUAGE.md](doc/DESIGN_LANGUAGE.md)**                                    | Web UI design system: compartment rules, spacing scale, typography, colour contrast, theme identity targets.            |
| **[doc/IDE_ONLY_CAPABILITIES.md](doc/IDE_ONLY_CAPABILITIES.md)**                        | Capabilities intentionally exclusive to the IDE extension (go-to-definition, code actions, data breakpoints).           |
| **[doc/LAUNCH_TEST.md](doc/LAUNCH_TEST.md)**                                            | Manual on-device test checklist for features automated tests cannot fully cover.                                        |
