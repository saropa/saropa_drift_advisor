 ...
 * the false-positive-suppression heuristics stay separate from the provider's
 * VS Code wiring (registration, code actions). Each function appends to the
 * caller's `issues` array.
 */
...
/** Flag a table that holds >50% of all database rows (data skew). */
...
/** Flag columns whose measured null rate is high enough to be a content gap. */
export async function checkHighNullRates(
  issues: IDiagnosticIssue[],
  ...
  for (const table of tables) {
    ...
    try {
      const nullCounts = await queryNullCounts(ctx, table);
      ...
      for (const col of table.columns) {
        ...
        if (nullPct >= HIGH_NULL_RATE_THRESHOLD) {
          ...
          if (dartCol && isNullByDesign(dartCol)) continue;
 ...
 */
function isNullByDesign(col: IDartColumn): boolean {
...
async function queryNullCounts(
  ...
  const nullExprs = table.columns.map(
    (c) => `SUM(CASE WHEN "${escapeSql(c.name)}" IS NULL THEN 1 ELSE 0 END) AS "${escapeSql(c.name)}_nulls"`,
  );

  const sql = `SELECT ${nullExprs.join(', ')} FROM "${escapeSql(table.name)}"`;
