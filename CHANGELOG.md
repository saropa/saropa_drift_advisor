...
- **Heartbeat / Watch screen (Feature 80, phase 1).** A live activity board in the web viewer (and the VS Code webview, which shares the same UI): every table with recorded activity appears as a card whose border glows on access — cool for reads, warm for writes — with per-table read/write counters, and the glow decays in under two seconds so rapid traffic burns brighter. An ECG-style monitor strip across the top aggregates all activity into a scrolling heart-rate trace with a live events-per-minute vital; idle databases show a calm flatline sweep. Untouched tables are never listed. Host-app writes are detected indirectly from row-count changes and labeled "Detected changes" (UPDATE-in-place is invisible in phase 1); the app's own reads are not visible — see `plans/80-heartbeat-phase2-host-capture.md` for the phase 2 capture design. Backed by a new in-memory `TableActivityTracker` and `GET /api/activity` endpoint (documented in `doc/API.md`): aggregates plus a bounded 200-event ring, internal/self-issued queries excluded so the board never glows from watching itself, and fully disabled under the monitoring kill switch.
...
- **Cursor-based ignore-diagnostic commands hardened against two races and one wasteful listener.** The QuickPick tie-break (used when several findings are equidistant from the cursor) now re-validates the picked finding against a fresh diagnostics read before inserting anything, so a directive can no longer land for a finding that changed or vanished while the pick was pending. The `onDidChangeDiagnostics` listener backing the right-click menu's visibility now filters the event's affected URIs down to the active editor's document before recomputing, instead of recomputing on every extension's diagnostics change in every open document.
- **Connection reliability phases 3–5.** Three structural improvements to how the extension handles an unreachable or newly-started debug server (`plans/connection-reliability-ongoing.md`):
  ...
  - **Circuit breaker fix: safety-timeout now feeds the breaker; half-open guards against concurrent probes.** Two bugs in the Phase 3 circuit breaker: (1) the Layer-2 safety timeout (`'Fetch timed out (safety)'`) — the dominant failure path on Windows — was not recognized by `isTransientError`, so the breaker never tripped when the server was genuinely unreachable on Windows; the breaker now counts all non-abort errors, not just the `isTransientError` subset. (2) Half-open state allowed every concurrent caller through simultaneously instead of one probe; a `_halfOpenProbeInFlight` guard now ensures exactly one, and a stale success arriving after the breaker has already reopened from a concurrent failure is ignored instead of silently closing the breaker.
  - **Per-endpoint circuit breaker isolation.** The single global `CircuitBreaker` is replaced by a `CircuitBreakerRegistry` that manages one breaker per endpoint group (extracted from the URL path: `/api/schema/…` → `schema`, `/api/dvr/…` → `dvr`, etc.). A failing analytics endpoint now trips only the `analytics` breaker — schema, DVR, and other groups continue unaffected. If the server is genuinely down, all groups trip independently within seconds. `resetAll()` force-closes every group when the user clicks Retry Discovery. The `instanceof CircuitBreakerOpenError` guard in the catch path now also checks `err.name` as a fallback against cross-module duplication.
- **Connection reliability hardening: telemetry + adb-forward supervision (campaign candidates D and E).** Two additive resilience layers for device debugging, where the link between the editor and a phone/emulator-hosted debug server drops most often:
  ...
  - **adb-forward supervision.** While a Dart/Flutter debug session runs, the extension watches `adb forward --list` every 15s for the expected `tcp:<port>` mapping and re-establishes it when it silently dies (device reconnect, adb server restart, editor crash mid-debug). Previously a dead forward was only healed reactively by an empty discovery scan — up to scan-interval + 60s throttle of unexplained "server lost". Only acts on a DROP (the forward must have been observed alive this session first), honors the existing 60s re-forward throttle, and preserves the once-per-session toast latch on recovery.
