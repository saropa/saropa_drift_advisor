...
- **Finish-report voice cleanup** — rewrote 23 persisted finish/plan report files (`plans/*.md`, `plans/history/**`, and the orphan-row/snapshot feature specs) from AI-session-narration voice into durable third-person engineering records: deleted the boilerplate "This work will be reviewed by another AI" lines, rewrote chat-quoting `Trigger` openers into objective problem/feature statements, and converted session-deixis ("the user asked", first-person "I/we", "this finish pass") to third person. Technical bodies (file lists, scope, test results, diffs) left verbatim. The generator fix (the shared `/finish` skill's Section 7 Voice block) is documented in [bugs/BUG_finish_skill_narration_voice.md](bugs/BUG_finish_skill_narration_voice.md); these were the already-written reports it could not retroactively correct.
