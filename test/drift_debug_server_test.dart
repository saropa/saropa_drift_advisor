...
void main() {
  ...
  test('DriftDebugErrorLogger callbacks never throw', () {
    ...
    expect(() => error(Exception('test'), StackTrace.current), returnsNormally);
  });
  ...
  test('DriftDebugErrorLogger.callbacks returns both callbacks', () {
    ...
    expect(() => c.error(Exception('e'), StackTrace.current), returnsNormally);
  });
  ...
  test(
    'DriftDebugErrorLogger.errorCallback with empty prefix does not throw',
    () {
      final error = DriftDebugErrorLogger.errorCallback(prefix: '');
      expect(() => error(Exception('e'), StackTrace.current), returnsNormally);
    },
  );
  ...
  group('param validation', () {
    test(
      'start with port -1 throws ArgumentError and server is not running',
      () async {
        expect(
    ...
    test(
      'start with port 70000 throws ArgumentError and server is not running',
      () async {
        expect(
          DriftDebugServer.start(
            query: (_) async => <Map<String, dynamic>>[],
    ...
    test(
      'start with only basicAuthUser (no password) throws ArgumentError',
      () async {
        expect(
          DriftDebugServer.start(
            query: (_) async => <Map<String, dynamic>>[],
    ...
    test(
      'start with only basicAuthPassword (no user) throws ArgumentError',
      () async {
        expect(
          DriftDebugServer.start(
            query: (_) async => <Map<String, dynamic>>[],
  ...
  group('hot restart resilience', () {
    ...
    test('shared:true allows re-binding the same port after stop+start '
        '(simulates hot restart)', () async {
    ...
    test('HttpServer.bind with shared:true succeeds when port is already '
        'bound by another shared listener', () async {
  ...
  group('defensive behavior: query and edge cases', () {
    ...
    test(
      'when query throws, GET /api/tables returns 500 and JSON error',
      () async {
        await DriftDebugServer.stop();
        await DriftDebugServer.start(
    ...
    test(
      'limit=0 or negative uses default limit, offset negative uses 0',
      () async {
        await DriftDebugServer.start(
          query: (String sql) async {
            if (sql.contains("type='table'") && sql.contains('ORDER BY name')) {
        ...
        final client = HttpClient();
        try {
          final reqLimit0 = await client.getUrl(
            Uri.parse(
  ...
  group('export endpoints', () {
    ...
    test(
      'GET /api/schema/diagram returns tables, columns, and foreign keys',
      () async {
        await DriftDebugServer.start(query: mockQuery, enabled: true, port: 0);
        final port = DriftDebugServer.port;
        expect(port, isNotNull);
        ...
        try {
          ...
          expect(columns.map((c) => (c as Map)['name']).toList(), [
            'id',
            'name',
          ]);
    ...
    test(
      'GET /api/table/<name> with limit and offset returns JSON array',
      () async {
        await DriftDebugServer.start(query: mockQuery, enabled: true, port: 0);
        final port = DriftDebugServer.port;
        expect(port, isNotNull);
    ...
    test(
      'POST /api/sql rejects body when Content-Type is not application/json',
      () async {
        await DriftDebugServer.start(query: mockQuery, enabled: true, port: 0);
        final port = DriftDebugServer.port;
        expect(port, isNotNull);
    ...
    test(
      'POST /api/sql accepts SELECT with keyword inside string literal',
      () async {
        await DriftDebugServer.start(query: mockQuery, enabled: true, port: 0);
        final port = DriftDebugServer.port;
        expect(port, isNotNull);
        ...
        try {
          ...
          req.headers.contentType = ContentType.json;
          req.write(
            jsonEncode(<String, String>{"sql": "SELECT 'INSERT' AS x"}),
          );
          final resp = await req.close();
          expect(resp.statusCode, HttpStatus.ok);
          final body = await resp.transform(utf8.decoder).join();
  ...
  group('secure dev tunnel auth', () {
    ...
    test(
      'request with query param token gets 401 (token in URL not supported)',
      () async {
        await DriftDebugServer.start(
          query: mockQuery,
          enabled: true,
    ...
    test(
      'request with Basic auth succeeds when basicAuthUser/Password set',
      () async {
        await DriftDebugServer.start(
          query: mockQuery,
          enabled: true,
  ...
  group('GET /api/database (raw SQLite file)', () {
    ...
    test(
      'returns 200 and database bytes when getDatabaseBytes provided',
      () async {
        const sqliteHeader = [
          0x53,
          0x51,
  ...
  group('Snapshot / time travel', () {
    ...
    test(
      'POST /api/snapshot captures state and GET returns metadata',
      () async {
        await DriftDebugServer.start(query: mockQuery, enabled: true, port: 0);
        final port = DriftDebugServer.port;
        expect(port, isNotNull);
        ...
        try {
          ...
          expect(
            (getData['snapshot'] as Map)['counts'],
            containsPair('items', 2),
          );
        } finally {
          client.close();
        }
    ...
    test(
      'GET /api/snapshot/compare returns diff when snapshot exists',
      () async {
        await DriftDebugServer.start(query: mockQuery, enabled: true, port: 0);
        final port = DriftDebugServer.port;
        expect(port, isNotNull);
  ...
  group('Database diff (queryCompare)', () {
    ...
    test(
      'GET /api/compare/report returns 501 when queryCompare not set',
      () async {
        await DriftDebugServer.start(query: mockQueryA, enabled: true, port: 0);
        final port = DriftDebugServer.port;
        expect(port, isNotNull);
    ...
    test(
      'GET /api/compare/report returns diff when queryCompare set',
      () async {
        await DriftDebugServer.start(
          query: mockQueryA,
          enabled: true,
        ...
        try {
          ...
          final itemsRow =
              (data['tableCounts'] as List).firstWhere(
    ...
    test(
      'GET /api/compare/report?format=download returns attachment',
      () async {
        await DriftDebugServer.start(
          query: mockQueryA,
          enabled: true,
  ...
  group('GET /api/schema/metadata', () {
    ...
    test('returns empty tables when change detection is disabled', () async {
      ...
      try {
        ...
        final metaReq = await client.get(
          'localhost',
          portValue,
          '/api/schema/metadata',
        );
        ...
        final diagramReq = await client.get(
          'localhost',
          portValue,
          '/api/schema/diagram',
        );
  ...
  group('change detection toggle', () {
    ...
    test('POST /api/change-detection toggles state', () async {
      ...
      try {
        ...
        final postReq = await client.post(
          'localhost',
          port!,
          '/api/change-detection',
        );
        ...
        // Re-enable.
        final reEnableReq = await client.post(
          'localhost',
          port,
          '/api/change-detection',
        );
