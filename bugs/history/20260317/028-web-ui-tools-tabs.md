...
- **Auto-run tools** — Index, Size, Perf, and Health tabs trigger their primary action on open (Analyze / Update / Scan) via `triggerToolButtonIfReady`; no duplicate run if already in progress or offline.
...
- `assets/web/app.js` — Tab API: `switchTab`, `openTool`, `closeToolTab`, `initTabsAndToolbar`; toolbar/tab event binding; `onTabSwitch` for schema/diagram lazy load and `triggerToolButtonIfReady` for Index/Size/Perf/Health auto-run on tab open; diagram init no longer requires sidebar toggle; schema toggle guarded when element missing; FK navigation uses `switchTab('sql')`.
