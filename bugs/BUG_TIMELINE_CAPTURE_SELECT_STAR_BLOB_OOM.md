...
> Status: Fixed (reported fixed by host-app maintainer — see Resolution). Retained as a record; verify the resolution note matches the landed extension change.
...
| **Relevant settings** | App side: `EnvType.DriftAdvisorEnabled` ON (default in debug). Extension side: timeline auto-capture enabled (default, `driftViewer.timeline.autoCapture`); `STARTUP_SWEEP_GRACE_MS = 6000`. |
...
The host-app maintainer reports the extension-side fix has been deployed and timeline auto-capture has been disabled as the interim mitigation. The fix should be verified against the suggestions above — specifically that BLOB columns are no longer materialized by the snapshot/branch/data-breakpoint `SELECT *` paths, not only the timeline auto-capture path. The host app itself required no change: the crash was driven entirely by the extension's capture query over the debug-only loopback, and the server is gated on `MainSettings.isDebugMode`, so release builds were never affected.
