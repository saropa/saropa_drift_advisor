...
3. **`assets/web/bundle.js` is committed and must stay deterministic** — esbuild minification is deterministic for a fixed version, so the sync gate proposed in `bugs/BUG_INFRA_BUNDLE_JS_HAS_NO_STALENESS_GATE.md` still works. But minify makes the committed diff unreadable, which argues for gating on a rebuild-and-compare rather than reviewing the diff.
...
4. Land the sync gate from `bugs/BUG_INFRA_BUNDLE_JS_HAS_NO_STALENESS_GATE.md` first or in the same change — otherwise step 1 and 2 can be committed without the regenerated bundle.
