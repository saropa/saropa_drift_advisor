...
4. **Real-time OS sync** — `matchMedia('prefers-color-scheme: dark').addEventListener('change')` updates the theme live when the OS setting changes (only when no explicit override is saved)
5. **Real-time VS Code sync** — `MutationObserver` on body classes and `data-vscode-theme-kind` attribute detects live VS Code theme switches in webview context

## Files Changed

- `lib/src/server/html_content.dart` — `applyTheme()` extracted, `detectVscodeTheme()` added, `initTheme()` rewritten with cascading detection, OS change listener, VS Code MutationObserver
