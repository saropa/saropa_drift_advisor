...
**Fourth pass — reflection hardening + `authScheme` feature (same day):** Per the finish-checklist reflection gate, the user selected both harden-reflection-items and implement-unrequested-feature. Hardening: documented (in code comments, not behavior changes) why `version`/`schemaVersion` are an acceptable pre-auth exposure for a local dev tool, and why `_hasBasicAuth`'s asymmetric guard (username must be non-empty, password only non-null) is intentional — it mirrors `AuthHandler.isAuthenticated` exactly, since a configured empty Basic password is a legitimate (if weak) dev-tunnel setup that `isAuthenticated` already lets through. Feature: added `ServerContext.authScheme` (`"basic"` | `"bearer"` | `null`) and a `jsonKeyAuthScheme` constant; both `GET /api/health` (reduced and full payloads) and `GET /api/` now carry `authScheme` alongside `authRequired`, letting a client pick the right `Authorization` scheme without parsing `WWW-Authenticate`. `GET /api/` is not auth-exempt, so the field there only helps a client already talking to several authenticated servers, not fresh discovery — documented as such. A code-review pass on this addition found no correctness issues but flagged the `authRequired`/`authScheme` pair as copy-pasted across three call sites; consolidated into `ServerContext.authStatusFields` (a `Map<String, dynamic>` spread via `...ctx.authStatusFields`) that all three (`sendHealth`'s two branches and `sendApiIndex`) now use. `doc/API.md` updated with the new field in both health response tables and the `/api/` index table/example. Test coverage: `authScheme` assertions added to the existing reduced/full/no-auth health tests, plus two new tests (`GET /api/ index includes authRequired/authScheme when authenticated`, `health reports authScheme "basic" when Basic auth is set`). Final combined suite: 164/164 passed.
