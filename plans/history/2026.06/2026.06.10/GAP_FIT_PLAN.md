...
- The parity sweep was complete (all high-impact and quick-win rows closed on
  both surfaces), but six rows were still `[ ]` plus an unstarted A-01…A-06
  doc-maintenance backlog. Archiving as-is would have buried live work, so the
  open rows were lifted into standalone per-feature plans before archival.
- This file `git mv`d from `plans/GAP_FIT_PLAN.md` to
  `plans/history/2026.06/2026.06.10/GAP_FIT_PLAN.md` (rename tracked; full
  history preserved).
- A short stub left at `plans/GAP_FIT_PLAN.md` (preserves the old path for any
  inbound link) points to this archive and the four splits.

**Splits created** (numbered to follow the repo's existing scheme, next free
after 70):

- `plans/71-website-dart-schema-scanning.md` — §5 row, from WP-A03.
- `plans/72-website-multiple-snapshots.md` — §8 row, from WP-A04.
- `plans/73-website-bulk-index-creation.md` — §11 row, from WP-A05.
- `plans/74-ide-only-capabilities.md` — §18 reclassification (go-to-definition,
  code actions, data breakpoints → intentionally IDE-only, not parity gaps),
  resolving A-02.

The remaining backlog (A-01 evidence coverage, A-06 parity release gate) governed
the now-archived tracking tables and retires with this archive.

**Verification:** repo-wide grep for `GAP_FIT_PLAN` across `*.md`/`*.ts`/`*.dart`
shows the only inbound references are the four new plans, each pointing correctly
at this archive path. No test files reference the plan. No broken links.

**Outstanding:** the three website parity features (plans 71–73) remain genuinely
unimplemented — they are now individually tracked rather than closed.
