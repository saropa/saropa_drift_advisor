...
Status: Fixed (advisor side) — all four findings resolved. Finding 1's ingest hook (`DriftDebugServer.reportAppQuery`) ships in the package; end-to-end perf data also requires the consumer app (Saropa Contacts) to wire the documented Drift `QueryInterceptor` — a one-time integration in that repo, not advisor code.
...
**Fixed — advisor ingest hook shipped (Finding 1):**

- **Finding 1 — `totalQueries: 0` in 95% of exports.** This was NOT a scoping/reset bug. The advisor only records queries that flow through its own instrumented path (`ServerContext.timedQuery`); the app's Drift queries execute in the app isolate and never reach the advisor. New `DriftDebugServer.reportAppQuery(sql, durationMs, rowCount, isWrite, error)` gives the app an in-process channel to forward its own query timings; they record via `ServerContext.recordAppTiming` tagged `source: "app"` (new `QueryTiming.appReported` flag), honor the kill switch and the 500-entry ring, and feed `TableActivityTracker` so app writes make the Finding 3 static-table ranking reliable. Design decisions taken (from `plans/history/2026.07/2026.07.24/61-app-query-timing-ingest.md`): in-process only (no HTTP — the server is in-process, no out-of-process consumer exists); explicit `app` origin flag (the interceptor has no meaningful caller frame); no per-query stack capture (cost refused). The package cannot ship the `QueryInterceptor` itself (zero-`package:drift` dependency contract), so a copy-paste recipe lives in the `reportAppQuery` doc comment. **Remaining, in the consumer repo:** Saropa Contacts must add that ~10-line interceptor for its exports to carry app timings — tracked there, not advisor code. Tests: `test/performance_handler_test.dart` (`recordAppTiming` group: source, kill switch, write-activity feed).
...
`totalQueries: 0` is not a scoping or reset bug: the advisor server records only queries issued through its own instrumented callback, and the consuming app's Drift queries execute in the app isolate without passing through it. No server-side filter can surface data the server never receives. The remedy is a query-timing ingest channel; because the debug server runs in-process inside the app, the correct shape is an in-process ingest hook plus a Drift `QueryInterceptor` helper, not the HTTP push the bug assumed. This is captured as a design in `plans/history/2026.07/2026.07.24/61-app-query-timing-ingest.md` with three open decisions, and the bug remains open pending that work.
...
## Finish Report Addendum (2026-07-24)

Two follow-ups landed after the initial report: hardening + a Finding 3 enhancement, then Finding 1 itself. This supersedes the "Why Finding 1 was not fixed" section above.

### Hardening and auto-suggest (Finding 3 enhancement)

- `_isIntrospection` was broadened from a PRAGMA-only match to also exclude statements referencing `sqlite_master`/`sqlite_schema`, so a future internal helper introspecting via the catalog on the instrumented path cannot re-pollute perf stats.
- The Finding 4 version stamp is now asserted on BOTH halves (the test registers a fake `saropa.drift-viewer` extension in the vscode mock), and `log-capture-api.buildSessionSnapshot` — a second sidecar-building path the first pass missed — now stamps `versions` too.
- The `outlier_check_hint` gained auto-suggest ranking: `TableActivityTracker.tablesWithObservedMutations()` (tables with an advisor write or an inferred host row-count change this session) is threaded into the detector, which puts only the no-mutation tables in the `staticTables` snippet and flags mutated tables as likely-not-static. Directional, not definitive — until app writes are observable it cannot see the app's own mutations. That gap is what Finding 1 closes.

### Finding 1 — app query-timing ingest (now fixed, advisor side)

The advisor never received the app's Drift queries because they run in the app isolate. The remedy is an in-process ingest channel — confirmed correct over an HTTP push because the debug server runs inside the app. `DriftDebugServer.reportAppQuery(sql, durationMs, rowCount, isWrite, error)` (static facade → instance → `ServerContext.recordAppTiming`) appends a timing tagged `source: "app"` via the new `QueryTiming.appReported` flag, honoring the kill switch and the 500-entry ring, and attributes the query to its tables so app writes drive the static-table ranking above (closing the loop). No per-query stack frame is parsed — the interceptor runs inside Drift, so a frame would be meaningless and `StackTrace.current` per query is a cost refused. The web stub gained a matching no-op (compile parity). Because the package must not depend on `package:drift`, it cannot ship the `QueryInterceptor` subclass; a copy-paste recipe lives in the `reportAppQuery` doc comment, and the consumer (Saropa Contacts) wires it in its own repo.

### Addendum verification

Dart tests executed and passing after each change: `performance_handler_test` (22, incl. `recordAppTiming` source/kill-switch/write-activity), `anomaly_detector_test` (62, incl. the two auto-suggest ranking tests), `handler_integration_test` (87). TypeScript remains compile-verified only (husky `tsc --noEmit` passes) — the VS Code test harness is not runnable in this environment.

## Finish Report Addendum 2 (2026-07-24)

Closes the two remaining discoverability gaps from the Finding 1 work.

- **The interceptor ships as tested code, not a doc snippet.** `example/lib/database/advisor_timing_interceptor.dart` is a real Drift `QueryInterceptor` (verified against the drift 2.31 `interceptor.dart` API) wired into the example database via `NativeDatabase(...).interceptWith(...)`. It lives in the example rather than `lib/` because the package must stay free of `package:drift`: the duck-typing design keeps it version-agnostic (no pub range coupling across drift majors) and usable with `drift_sqlite_async`/custom executors that a hard dependency would exclude. Example dev-deps never reach consumers. Tested in `example/test/advisor_timing_interceptor_test.dart` (transparency + error propagation).
- **The symptom advertises its own cure.** `PerformanceHandler.getPerformanceData` attaches a one-line `hint` field whenever no `source:"app"` query has been captured — i.e. the interceptor is not installed — telling the developer to install it. It rides into the exported sidecar (where `totalQueries:0` is read) and clears itself once any app query lands. A README subsection and `doc/API.md` entry document the feature at the pub.dev front door.
- **Facade coverage + doc fix.** `test/drift_debug_server_test.dart` gains an end-to-end `reportAppQuery` test (start server → report → assert `source:"app"` in `/api/analytics/performance`) plus a not-running no-op test. `TableActivityTracker.tablesWithObservedMutations` doc comment was corrected — it claimed app writes are invisible, which the interceptor path now contradicts.

### Addendum 2 verification

`performance_handler_test`, `anomaly_detector_test`, `handler_integration_test`, `drift_debug_server_test` (67, incl. the new facade tests), and `example/test/advisor_timing_interceptor_test.dart` all pass. The external saropa_lints auto-fixes present in the same working tree (a behavior-preserving `router.dart` dispatch-loop refactor and `// ignore:` rationale additions) were reviewed and confirmed non-behavioral; the full HTTP suite exercises the refactored router.
