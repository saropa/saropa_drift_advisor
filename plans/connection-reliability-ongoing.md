...
- Extension activated before commands registered → "command not found"
...
2. **No connection integration tests** — The extension has unit tests for individual components (URI parsing, VM Service client, etc.) but no tests that simulate the full connection lifecycle: extension activates → discovery scans → server found → tree loads → user clicks button → data appears. Regressions are only caught by manual testing.
