...
<div class="settings-panel">
  ...
  function temporalWhere(q, target) {
    ...
    if (/chang|modif|updat|edit/i.test(q)) {
      col = dateCols.find(function(c) {
        return /updat|modif|chang|edit/i.test(c.name);
      });
    } else if (/creat|added|\bnew\b|register|insert/i.test(q)) {
      col = dateCols.find(function(c) {
        return /creat|add|insert|register/i.test(c.name);
      });
    }
    if (!col) col = dateCols[0];
    const isEpoch = /int/i.test(col.type);
    const dateExpr = isEpoch ? 'date("' + col.name + `", 'unixepoch', 'localtime')` : 'date("' + col.name + '")';
