...
export function loadSchemaView() {
   ...
   return;
      ...
      fetch('/api/schema', S.authOpts())
        ...
        .catch(e => { content.innerHTML = '<p class="meta">' + vt('viewer.schema.error') + '</p><pre>' + esc(String(e)) + '</pre>'; });
    }
...
export function renderSchemaContent(container, schema) {
      ...
      if (scope === 'both') {
        container.innerHTML = '<div class="search-section-collapsible expanded">' +
          ...
          '<div class="collapsible-body"><p class="meta">' + vt('viewer.schema.selectTablePrompt') + '</p></div>' +
          '</div>';
        ...
        if (dataSection && S.currentTableName && S.currentTableJson !== null) {
          ...
          if (headerEl) headerEl.textContent = vt('viewer.schema.tableData.headingNamed', S.currentTableName);
